...
async def admin_withdraw_decision_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if action == "admin_wd_approve":
        # به‌جای نهایی‌کردن فوری، اول لینک تراکنش رو از ادمین می‌خوایم
        # (finalize واقعی تو handlers_support.finalize_withdraw_approval انجام می‌شه)
        import handlers_support
        await handlers_support.start_withdraw_tx_link_flow(update, context, request_id)
        return
        ...
        await post_to_orders_channel(
            context.bot,
            f"❌ سفارش #{request_id} لغو شد.\nنوع: برداشت TON\nآدرس اشتباه بود یا مشکل داشت — مبلغ برگشت داده شد.",
        )
        try:
            await context.bot.send_message(
                req["user_id"],
                f"❌ درخواست برداشت شما (#{request_id}) رد شد — آدرس واردشده مشکل داشت یا "
                f"تایید نشد. {req['liber_amount']} LIBER به حساب شما بازگشت.\n"
                "برای جزئیات بیشتر می‌تونید از «📞 تماس با پشتیبانی» استفاده کنید.",
            )
        except TelegramError:
            pass
...
async def admin_router(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # --- fallback: سفارش‌های گیفت استارز و بوست کانال (پارت handlers_competition_boost.py) ---
    if (data == "admin_pending_giftboost" or data.startswith("admin_gb_done:") or data.startswith("admin_gb_reject:")
            or data == "admin_pending_channelboost" or data.startswith("admin_cb_done:") or data.startswith("admin_cb_reject:")):
